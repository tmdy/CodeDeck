...
const profileApi = {
  ...
  updateProfileRuntimeSettings: (profileKey: string, patch: unknown): Promise<unknown> =>
    ipcRenderer.invoke("profile:update-runtime-settings", profileKey, patch),
  ...
  updateSiteBalanceSessionAutoCheckin: (
    baseUrl: string,
    sessionId: string,
    enabled: boolean,
  ...
  getSessionDetail: (request: unknown): Promise<unknown> =>
    ipcRenderer.invoke("session:get-detail", request),
  refreshSessions: (provider: string): Promise<unknown> =>
    ipcRenderer.invoke("session:refresh", provider),
  onSessionCatalogUpdated: (callback: (update: unknown) => void): (() => void) => {
    const handler = (_event: unknown, update: unknown) => callback(update);
    ipcRenderer.on("session:catalog-updated", handler);
    return () => ipcRenderer.removeListener("session:catalog-updated", handler);
  },
  ...
  runCheckin: (baseUrl: string, sessionId: string): Promise<unknown> =>
    ipcRenderer.invoke("checkin:run-account", baseUrl, sessionId),
  runEnabledCheckins: (): Promise<unknown> =>
    ipcRenderer.invoke("checkin:run-enabled"),
  openCheckinVerification: (baseUrl: string, sessionId: string): Promise<unknown> =>
    ipcRenderer.invoke("checkin:open-verification", baseUrl, sessionId),
  getCheckinSnapshot: (): Promise<unknown> =>
    ipcRenderer.invoke("checkin:get-snapshot"),
  ...
  onCheckinProgress: (
    callback: (accountKey: string, state: unknown) => void,
  ): (() => void) => {
    const handler = (_event: unknown, accountKey: string, state: unknown) =>
      callback(accountKey, state);
    ipcRenderer.on("checkin:progress", handler);
    return () => {
      ipcRenderer.removeListener("checkin:progress", handler);
    };
  },
  onCheckinScheduleChanged: (
    callback: (nextScheduledAt: string) => void,
  ): (() => void) => {
    const handler = (_event: unknown, nextScheduledAt: string) =>
      callback(nextScheduledAt);
    ipcRenderer.on("checkin:schedule-changed", handler);
    return () => {
      ipcRenderer.removeListener("checkin:schedule-changed", handler);
    };
  },
...
const terminalApi = {
  ...
  updateAutoContinueConfig: (sessionId: string, patch: unknown): Promise<void> =>
    ipcRenderer.invoke("terminal:update-auto-continue", sessionId, patch),
