...
import { getNextSessionCursor } from "./shared/session-pagination.js";
import { itemKey } from "./shared/profile/keys-internal.js";
import {
  PROVIDER_CLAUDE,
  PROVIDER_CODEX,
  defaultRuntimeSettings,
  normalizeRuntimeTerminalMode,
  resolveClaudeModelAliasMode,
  shouldRecommendClaudeSingleModelCompatibility,
} from "./shared/profile/types.js";
import { normalizeGlobalPermissions, type PermissionPreset, type ProfilePermissions } from "./shared/profile/permissions.js";
import {
...
type ProfilesSessionsCacheEntry = {
...
function buildLaunchPanelTerminalSummary(
  ...
  const terminalSettings = parameterSettings?.terminal;
  ...
  return `Codex 终端：受监控独立窗口（自动继续已启用，最多 ${terminalSettings?.auto_continue_limit ?? 1} 次）`;
}

function resolveProfileTerminalMode(
  runtime: RuntimeSettings | undefined,
  parameterSettings: ParameterSettings | undefined,
): RuntimeTerminalMode {
  const runtimeMode = normalizeRuntimeTerminalMode(runtime?.terminal_mode);
  if (runtimeMode) {
    return runtimeMode;
  }
  return parameterSettings?.terminal?.terminal_mode ?? "monitored";
}
...
function sessionSourceFromSummary(session?: SessionSummary): LaunchSessionSource | undefined {
  if (!session?.source_kind && !session?.source_home && !session?.source_file_relative_path) {
    return undefined;
  }
  return {
...
function sameSessionRequest(left: ListSessionsRequest | null, right: ListSessionsRequest): boolean {
  return left?.provider === right.provider
    ...
    && (left.detail ?? "full") === (right.detail ?? "full");
}

function normalizeSessionPageResult(result: SessionPageResult | unknown[]): SessionPageResult {
  if (Array.isArray(result)) {
    const sessions = result as SessionSummary[];
    return {
      sessions,
      ...(getNextSessionCursor(sessions) ? { next_cursor: getNextSessionCursor(sessions) } : {}),
      has_more: result.length >= HISTORY_PAGE_SIZE,
      catalog_state: "ready",
      partial: false,
      revision: 0,
    };
...
function mergeSessionSummaries(
  prioritySessions: SessionSummary[],
  existingSessions: SessionSummary[],
  maximum?: number,
): SessionSummary[] {
  const merged = new Map<string, SessionSummary>();
  for (const session of [...prioritySessions, ...existingSessions]) {
    const key = getSessionFavoriteKey(session);
    ...
    merged.set(key, existing ? {
      ...
      user_prompts: session.user_prompts?.length ? session.user_prompts : existing.user_prompts,
      conversation_excerpts: session.conversation_excerpts?.length
  ...
  return maximum === undefined ? sorted : sorted.slice(0, maximum);
}

function profilesSessionsCacheKey(request: ListSessionsRequest): string {
...
function reusableProfilesSessionsCacheKey(request: ListSessionsRequest): string {
  return JSON.stringify({
    provider: request.provider.trim().toLowerCase(),
    scope: request.scope,
    cwd: request.cwd?.trim().replace(/\\/g, "/").replace(/\/$/, "").toLowerCase() ?? "",
    limit: request.limit ?? 0,
    offset: request.offset ?? 0,
    cursor: request.cursor ?? "",
    detail: request.detail ?? "full",
...
function resolveCachedProfilesSelectedSessionId(
  sessions: SessionSummary[],
  selectedSessionId: string,
): string {
  return selectedSessionId && sessions.some((session) => session.session_id === selectedSessionId)
    ? selectedSessionId
    : "";
}
...
function App() {
  ...
  const [checkinActionBusy, setCheckinActionBusy] = useState(false);
  const [checkinBatchBusy, setCheckinBatchBusy] = useState(false);
  ...
  const [profilesSessionsRefreshing, setProfilesSessionsRefreshing] = useState(false);
  const [profilesCatalogState, setProfilesCatalogState] = useState<SessionCatalogState>("ready");
  ...
  const [historyIsRefreshing, setHistoryIsRefreshing] = useState(false);
  const [historyHasMore, setHistoryHasMore] = useState(false);
  const [historyNextCursor, setHistoryNextCursor] = useState<string | undefined>();
  const [historyCatalogState, setHistoryCatalogState] = useState<SessionCatalogState>("ready");
  ...
  const [historyPrefetchedNextCursor, setHistoryPrefetchedNextCursor] = useState<string | undefined>();
  ...
  const historyHydratedRequestRef = useRef<ListSessionsRequest | null>(null);
  const historyLoadingRequestRef = useRef<ListSessionsRequest | null>(null);
  const historyWarmupRequestRef = useRef<ListSessionsRequest | null>(null);
  ...
  const profilesManualRefreshRequestRef = useRef<ListSessionsRequest | null>(null);
  const historyManualRefreshRequestRef = useRef<ListSessionsRequest | null>(null);
  const historyPrefetchedCursorRef = useRef<string | null>(null);
  const historyPaginationModeRef = useRef<"cursor" | "offset">("cursor");
  const sessionCatalogRevisionRef = useRef<Record<string, number>>({});
  const historyRequestSeqRef = useRef(0);
  const historySessionDetailRequestSeqRef = useRef(0);
  const historySelectedSessionIdRef = useRef("");
  const profilesSessionsRequestSeqRef = useRef(0);
  const profilesSessionDetailRequestSeqRef = useRef(0);
  ...
  const profilesSelectedSessionIdRef = useRef("");
  const profilesSessionsCacheRef = useRef(new Map<string, ProfilesSessionsCacheEntry>());
  const reusableProfilesSessionsCacheRef = useRef(new Map<string, ProfilesSessionsCacheEntry>());
  ...
  const currentCheckinAccount = useMemo(
    () => draftSiteBalanceSessions.find((session) => session.id === draftBalanceSessionSelection) ?? null,
    [draftBalanceSessionSelection, draftSiteBalanceSessions],
  );
  const enabledCheckinAccountCount = useMemo(
    () => Object.values(siteBalanceSessionsByBaseUrl)
      .flat()
      .filter((session) => session.auto_checkin_enabled === true).length,
    [siteBalanceSessionsByBaseUrl],
  );
  const currentCheckinAccountKey = currentCheckinAccount
    ? buildCheckinAccountKey(normalizeBalanceBaseUrl(currentCheckinAccount.base_url), currentCheckinAccount.id)
    : "";
  const currentCheckinState = currentCheckinAccountKey
    ? state?.checkin_states_by_account[currentCheckinAccountKey] ?? null
    : null;
  const anyCheckinRunning = Object.values(state?.checkin_states_by_account ?? {})
    .some((checkinState) => (
      checkinState.status === "running" || checkinState.status === "verifying"
    ));
  ...
  useEffect(() => {
    const profileKey = state?.selected_profile_key ?? "";
    const runtime = profileKey ? state?.runtime_by_profile[profileKey] : undefined;
    setProfilesLaunchMonitorModeEnabled(
      resolveProfileTerminalMode(runtime, state?.parameter_settings) === "monitored",
    );
  }, [
    state?.selected_profile_key,
    state?.runtime_by_profile,
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      clearProfilesSessionsState({ clearCache: true });
  ...
  function rememberProfilesSessions(
    request: ListSessionsRequest,
    sessions: SessionSummary[],
    selectedSessionId = profilesSelectedSessionIdRef.current,
  ) {
    const resolvedSelectedSessionId = resolveCachedProfilesSelectedSessionId(sessions, selectedSessionId);
    profilesSessionsCacheRef.current.set(profilesSessionsCacheKey(request), {
      request,
      sessions,
      selectedSessionId: resolvedSelectedSessionId,
    });
    reusableProfilesSessionsCacheRef.current.set(reusableProfilesSessionsCacheKey(request), {
      request,
      sessions,
      selectedSessionId: resolvedSelectedSessionId,
    });
  ...
  function applyCachedProfilesSessions(request: ListSessionsRequest): boolean {
    const cached = profilesSessionsCacheRef.current.get(profilesSessionsCacheKey(request));
    if (!cached || !sameSessionRequest(cached.request, request)) {
      return false;
    }

    const nextSelectedSessionId = resolveCachedProfilesSelectedSessionId(
      cached.sessions,
      cached.selectedSessionId,
    );
    ...
    profilesSelectedSessionIdRef.current = nextSelectedSessionId;
    setProfilesSessions(cached.sessions);
    setProfilesSelectedSessionId((current) => (
      current === nextSelectedSessionId ? current : nextSelectedSessionId
    ));
    setProfilesSessionsLoading(false);
    return true;
  }

  function applyReusableProfilesSessions(request: ListSessionsRequest): boolean {
    const cached = reusableProfilesSessionsCacheRef.current.get(reusableProfilesSessionsCacheKey(request));
    if (!cached || (cached.request.profile_key ?? "") === (request.profile_key ?? "")) {
      return false;
    }

    const nextSelectedSessionId = resolveCachedProfilesSelectedSessionId(
      cached.sessions,
      profilesSelectedSessionIdRef.current,
    );
    markProfilesSessionsHydrated(request.provider, request);
    profilesSelectedSessionIdRef.current = nextSelectedSessionId;
    setProfilesSessions(cached.sessions);
    setProfilesSelectedSessionId(nextSelectedSessionId);
  ...
  async function saveCurrentProfile(options: {
    ...
    try {
      ...
      const runtimeSnapshot = buildRuntimeSettingsFromDraft(snapshot);
      const currentRuntime = state?.selected_profile_key
        ? state.runtime_by_profile[state.selected_profile_key]
        : undefined;
      const terminalMode = normalizeRuntimeTerminalMode(currentRuntime?.terminal_mode);
      await window.profileManager.saveProfile(
        editingKey,
        draft,
        terminalMode ? { ...runtimeSnapshot, terminal_mode: terminalMode } : runtimeSnapshot,
      );
  ...
  async function persistReasoningEffortOnlyChange(snapshot = currentDraftSnapshot()): Promise<boolean> {
    if (!window.profileManager || !state?.selected_profile_key || !editingKey) {
      return false;
    }
    if (!hasOnlyProfileDraftReasoningEffortChange(snapshot, editorBaseline)) {
      return false;
    }
    return saveCurrentProfile({ snapshot, showSuccess: false });
  }
  ...
  function clearHistoryState() {
    ...
    historySessionDetailRequestSeqRef.current += 1;
    historySelectedSessionIdRef.current = "";
    latestHistoryRequestRef.current = null;
    historyHydratedRequestRef.current = null;
    historyLoadingRequestRef.current = null;
    historyWarmupRequestRef.current = null;
    setHistorySessions([]);
    setHistorySelectedSessionId("");
    setHistoryPreview(emptyPreview());
    setHistoryIsLoading(false);
    setHistoryIsRefreshing(false);
    setHistoryHasMore(false);
    setHistoryNextCursor(undefined);
    setHistoryCatalogState("ready");
    setHistoryPrefetchedSessions([]);
    setHistoryPrefetchedHasMore(false);
    setHistoryPrefetchedNextCursor(undefined);
    setHistoryIsPrefetching(false);
    latestHistoryPrefetchRequestRef.current = null;
    historyPrefetchedCursorRef.current = null;
  }

  function clearProfilesSessionsState(options: {
    loading?: boolean;
    resetHydration?: boolean;
    clearCache?: boolean;
  } = {}) {
    profilesSessionsRequestSeqRef.current += 1;
    profilesSessionDetailRequestSeqRef.current += 1;
    latestProfilesSessionsRequestRef.current = null;
    if (options.clearCache) {
      profilesSessionsCacheRef.current.clear();
      reusableProfilesSessionsCacheRef.current.clear();
    }
    ...
    setProfilesSessionsLoading((current) => {
      ...
      setProfilesSessionsRefreshing(false);
      setProfilesCatalogState("ready");
  }
  ...
  async function resolveUnsavedProfileChanges(): Promise<boolean> {
    ...
    if (hasOnlyProfileDraftReasoningEffortChange(snapshot, editorBaseline)) {
      return persistReasoningEffortOnlyChange(snapshot);
    }
    if (hasOnlyProfileDraftBalanceSessionChange(snapshot, editorBaseline)) {
  ...
  async function handleLaunch(
    ...
    if (hasProfileDraftChanges(snapshot, editorBaseline)) {
      if (hasOnlyProfileDraftCwdChange(snapshot, editorBaseline)) {
        ...
        const latest = await loadData();
        launchState = latest.state;
      } else if (hasOnlyProfileDraftReasoningEffortChange(snapshot, editorBaseline)) {
        const saved = await persistReasoningEffortOnlyChange(snapshot);
        if (!saved) {
          return;
        }
        const latest = await loadData();
    ...
    const requestedTerminalMode = profilesLaunchMonitorModeEnabled ? "monitored" : "direct";
    ...
    try {
      ...
      const prefersMonitoredTerminal = requestedTerminalMode === "monitored";
      if (launchResult?.monitoringActive) {
        setSuccessMessage("已打开受监控终端窗口。");
      } else if (prefersMonitoredTerminal && launchResult?.terminalMode === "direct") {
  ...
  async function handleProfilesLaunchMonitorModeChange(enabled: boolean) {
    const terminalMode: RuntimeTerminalMode = enabled ? "monitored" : "direct";
    setProfilesLaunchMonitorModeEnabled(enabled);
    if (!state?.selected_profile_key) {
      return;
    }

    const profileKey = state.selected_profile_key;
    const provider = state.selected_provider as "claude" | "codex";
    const previousRuntime = state.runtime_by_profile[profileKey] ?? defaultRuntimeSettings(provider);
    ...
    setState((current) => current ? {
      ...current,
      runtime_by_profile: {
        ...current.runtime_by_profile,
        [profileKey]: optimisticRuntime,
      },
    } : current);

    try {
      const savedRuntime = await window.profileManager?.updateProfileRuntimeSettings?.(
        profileKey,
        { terminal_mode: terminalMode },
      );
      if (!savedRuntime) {
      ...
      setState((current) => current ? {
        ...
        runtime_by_profile: {
          ...current.runtime_by_profile,
          [profileKey]: previousRuntime,
        },
      } : current);
      if (state.selected_profile_key === profileKey) {
        setProfilesLaunchMonitorModeEnabled(
          resolveProfileTerminalMode(
            previousRuntime,
            state.parameter_settings,
          ) === "monitored",
        );
      }
      setErrorMessage(err instanceof Error ? err.message : "保存监控模式失败");
    }
  }
  ...
  async function handleLoadProfilesSessions(
    ...
    if (options.source !== "manual" && options.source !== "followup" && applyCachedProfilesSessions(request)) {
      return;
    }
    ...
    latestProfilesSessionsRequestRef.current = request;
    if (options.source === "manual") {
      profilesManualRefreshRequestRef.current = request;
    }
    setProfilesSessionsRefreshing(options.source === "manual");
    ...
    try {
      if (options.source === "manual") {
        await window.profileManager.refreshSessions(request.provider);
      }
      ...
      const page = normalizeSessionPageResult(result);
      sessionCatalogRevisionRef.current[request.provider] = Math.max(
        sessionCatalogRevisionRef.current[request.provider] ?? 0,
        page.revision,
      );
      const loadedSessions = page.sessions;
      const nextSessions = shouldKeepExisting && (options.source === "manual" || options.source === "followup")
        ? mergeSessionSummaries(
            loadedSessions,
            profilesSessions,
            Math.max(HISTORY_PAGE_SIZE, profilesSessions.length),
          )
        : loadedSessions;
      setProfilesCatalogState(page.catalog_state);
      const nextSelectedSessionId = resolveCachedProfilesSelectedSessionId(
        nextSessions,
        profilesSelectedSessionIdRef.current,
      );
      rememberProfilesSessions(request, nextSessions, nextSelectedSessionId);
      ...
      if (options.source === "manual" && profilesManualRefreshRequestRef.current === request) {
        profilesManualRefreshRequestRef.current = null;
      }
    }
  }

  async function resolveSavedCheckinAccount() {
    ...
    const latest = await loadData();
    const profile = latest.profiles.find((item) => itemKey(item) === latest.state.selected_profile_key);
    if (!profile?.balance_session_id) {
      setErrorMessage("请先选择并保存一个后台账号");
      return null;
    }
    const account = getSiteBalanceSessionsForBaseUrl(
      latest.siteBalanceSessionsByBaseUrl,
      profile.url,
    ).find((session) => session.id === profile.balance_session_id);
    if (!account) {
      setErrorMessage("所绑定的后台账号已不存在，请重新选择");
      return null;
    }
    return account;
  }

  async function handleRunCheckin() {
    if (!window.profileManager?.runCheckin || checkinActionBusy || checkinBatchBusy) return;
    const account = await resolveSavedCheckinAccount();
    if (!account) return;
    setCheckinActionBusy(true);
    try {
      const result = await window.profileManager.runCheckin(account.base_url, account.id) as CheckinResult;
      if (result.status === "success" || result.status === "already_checked") {
        setSuccessMessage(result.message);
      } else {
        const diagnosticSuffix = result.diagnostic_run_id
          ? `（诊断编号：${formatCheckinDiagnosticId(result.diagnostic_run_id)}）`
          : "";
        setErrorMessage(`${result.message}${diagnosticSuffix}`);
      }
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "签到失败");
    } finally {
      setCheckinActionBusy(false);
    }
  }

  async function handleRunEnabledCheckins() {
    if (!window.profileManager?.runEnabledCheckins || checkinActionBusy || checkinBatchBusy) return;
    setCheckinBatchBusy(true);
    try {
      const summary = await window.profileManager.runEnabledCheckins() as CheckinBatchSummary;
      const message = `批量签到完成：成功 ${summary.success + summary.already_checked}，需处理 ${summary.manual_required}，失败 ${summary.failed + summary.unsupported}`;
      if (summary.failed || summary.manual_required) {
        setErrorMessage(message);
      } else {
        setSuccessMessage(message);
      }
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "批量签到失败");
    } finally {
      setCheckinBatchBusy(false);
    }
  }

  async function handleToggleAutoCheckin(enabled: boolean) {
    if (
      !window.profileManager?.updateSiteBalanceSessionAutoCheckin
      || checkinActionBusy
      || checkinBatchBusy
    ) return;
    const account = await resolveSavedCheckinAccount();
    ...
    setCheckinActionBusy(true);
    try {
      await window.profileManager.updateSiteBalanceSessionAutoCheckin(
        account.base_url,
        account.id,
        enabled,
      );
      await loadData();
      setSuccessMessage(enabled ? "已开启自动签到" : "已关闭自动签到");
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "更新自动签到设置失败");
    } finally {
      setCheckinActionBusy(false);
    }
  }

  async function handleOpenManualCheckin() {
    if (
      !window.profileManager?.openCheckinVerification
      || checkinActionBusy
      || checkinBatchBusy
    ) return;
    const account = await resolveSavedCheckinAccount();
    ...
    setCheckinActionBusy(true);
    try {
      const result = await window.profileManager.openCheckinVerification(
        account.base_url,
        account.id,
      ) as CheckinVerificationResult;
      if (result.status === "success" || result.status === "already_checked") {
        setSuccessMessage(result.message);
      } else {
      ...
      setErrorMessage(err instanceof Error ? err.message : "打开验证窗口失败");
    } finally {
      setCheckinActionBusy(false);
    }
  }

  function handleSelectProfilesSession(sessionId: string) {
    profilesSelectedSessionIdRef.current = sessionId;
    setProfilesSelectedSessionId(sessionId);
    const hydratedRequest = profilesSessionsHydratedRequestRef.current;
    if (!hydratedRequest) {
      return;
    }
    const cacheKey = profilesSessionsCacheKey(hydratedRequest);
    const cached = profilesSessionsCacheRef.current.get(cacheKey);
    if (!cached) {
      return;
    }
    profilesSessionsCacheRef.current.set(cacheKey, {
      ...cached,
      selectedSessionId: resolveCachedProfilesSelectedSessionId(cached.sessions, sessionId),
    });
    const selectedSession = cached.sessions.find((session) => session.session_id === sessionId);
    if (selectedSession) {
      void loadProfilesSessionDetail(hydratedRequest, selectedSession);
    }
  }

  async function loadProfilesSessionDetail(request: ListSessionsRequest, session: SessionSummary) {
    if (!window.profileManager?.getSessionDetail) {
      return;
    }
    if (session.conversation_excerpts?.length) {
      return;
    }

    const detailSeq = ++profilesSessionDetailRequestSeqRef.current;
    try {
      const detail = await window.profileManager.getSessionDetail({
        ...request,
        detail: "full",
      ...
      if (
        detailSeq !== profilesSessionDetailRequestSeqRef.current
        || profilesSelectedSessionIdRef.current !== session.session_id
        || !sameSessionRequest(profilesSessionsHydratedRequestRef.current, request)
      ) {
        return;
      }
      const detailedSession = detail as SessionSummary | null;
      if (!detailedSession) {
        return;
      }
      setProfilesSessions((current) => current.map((candidate) => (
        candidate.session_id === session.session_id
          ? { ...candidate, ...detailedSession }
          : candidate
      )));
      const cacheKey = profilesSessionsCacheKey(request);
      const cached = profilesSessionsCacheRef.current.get(cacheKey);
      if (cached) {
        profilesSessionsCacheRef.current.set(cacheKey, {
          ...cached,
          sessions: cached.sessions.map((candidate) => (
            candidate.session_id === session.session_id
              ? { ...candidate, ...detailedSession }
              : candidate
          )),
  ...
  function handleSelectHistorySession(sessionKey: string) {
    historySelectedSessionIdRef.current = sessionKey;
    setHistorySelectedSessionId(sessionKey);
    const session = visibleHistorySessions.find(
      (candidate) => getSessionFavoriteKey(candidate) === sessionKey,
    );
    if (!session || session.conversation_excerpts?.length || !window.profileManager?.getSessionDetail) {
      return;
    }
    const requestSeq = ++historySessionDetailRequestSeqRef.current;
    void window.profileManager.getSessionDetail({
      ...
      if (
        requestSeq !== historySessionDetailRequestSeqRef.current
        || historySelectedSessionIdRef.current !== sessionKey
        || !result
      ) {
        return;
      }
      const detail = result as SessionSummary;
      setHistorySessions((current) => current.map((candidate) => (
        getSessionFavoriteKey(candidate) === sessionKey
          ? { ...candidate, ...detail, source_kind: candidate.source_kind, source_home: candidate.source_home }
          : candidate
      )));
    }).catch(() => {
      // Detail loading enhances the selected summary only.
    });
  }

  function buildHistorySessionsRequest(
    ...
    return {
      ...
      profile_key: nextState.selected_profile_key,
      limit: HISTORY_PAGE_SIZE,
      offset: historyPaginationModeRef.current === "offset" ? offset : 0,
      ...(cursor && historyPaginationModeRef.current === "cursor" ? { cursor } : {}),
    };
  }
  ...
  async function prefetchHistorySessions(
    nextState: LocalState | null = state,
    cursor = historyNextCursor,
    offset = historySessions.length,
  ) {
    if (!window.profileManager || !nextState || !cursor) {
      return;
    }
    if (historyPrefetchedCursorRef.current === cursor) {
      return;
    }

    const request = buildHistorySessionsRequest(nextState, cursor, offset);
    ...
    try {
      ...
      const page = normalizeSessionPageResult(result);
      historyPaginationModeRef.current = Array.isArray(result) ? "offset" : "cursor";
      sessionCatalogRevisionRef.current[request.provider] = Math.max(
        sessionCatalogRevisionRef.current[request.provider] ?? 0,
        page.revision,
      );
      const nextSessions = page.sessions.filter((session) => session.provider === request.provider);
      setHistoryPrefetchedSessions(nextSessions);
      setHistoryPrefetchedHasMore(page.has_more);
      setHistoryPrefetchedNextCursor(page.next_cursor);
      setHistoryCatalogState(page.catalog_state);
      setHistoryHasMore(nextSessions.length > 0);
      setHistoryIsPrefetching(false);
      historyPrefetchedCursorRef.current = cursor;
    } catch {
      if (!sameSessionRequest(latestHistoryPrefetchRequestRef.current, request)) {
        return;
      }
  ...
  async function handleLoadHistorySessions(
    nextState: LocalState | null = state,
    options: { append?: boolean; refresh?: boolean; background?: boolean; catalogUpdate?: boolean } = {},
  ) {
    ...
    const shouldPreserveExisting = options.append || options.refresh || options.catalogUpdate;
    const previousSessions = shouldPreserveExisting ? historySessions : [];
    const request = buildHistorySessionsRequest(
      nextState,
      options.append ? historyNextCursor : undefined,
      options.append ? historySessions.length : 0,
    );
    const canReuseLoadedRequest = !options.append
      && !options.refresh
      && !options.catalogUpdate
      && sameSessionRequest(historyHydratedRequestRef.current, request);
    if (canReuseLoadedRequest) {
      setHistoryIsLoading(false);
      return;
    }
    const isSameRequestLoading = !options.append
      && !options.refresh
      && sameSessionRequest(historyLoadingRequestRef.current, request);
    if (isSameRequestLoading) {
      return;
    }
    ...
    if (options.refresh === true) {
      historyManualRefreshRequestRef.current = request;
    }
    setHistoryIsRefreshing(options.refresh === true);
    if (!options.append) {
      clearHistoryPrefetchState();
    }
    if (!options.catalogUpdate && !options.background) {
    ...
    try {
      ...
      const page = normalizeSessionPageResult(result);
      historyPaginationModeRef.current = Array.isArray(result) ? "offset" : "cursor";
      sessionCatalogRevisionRef.current[provider] = Math.max(
        sessionCatalogRevisionRef.current[provider] ?? 0,
        page.revision,
      );
      const nextSessions = page.sessions.filter((session) => session.provider === provider);
      const combinedSessions = options.append
        ? mergeSessionSummaries(previousSessions, nextSessions)
        : shouldPreserveExisting
          ? mergeSessionSummaries(
              nextSessions,
              previousSessions,
              Math.max(HISTORY_PAGE_SIZE, previousSessions.length),
            )
          : nextSessions;
      if (!options.append) {
        historyHydratedRequestRef.current = request;
      }
      setHistorySessions(combinedSessions);
      const nextCursor = options.append
        ? page.next_cursor
        : shouldPreserveExisting
          ? getNextSessionCursor(combinedSessions)
          : page.next_cursor;
      ...
      if (page.has_more && nextCursor) {
        void prefetchHistorySessions(nextState, nextCursor, combinedSessions.length);
      }
    } catch (err) {
      if (requestSeq !== historyRequestSeqRef.current || !sameSessionRequest(latestHistoryRequestRef.current, request)) {
        return;
      }
      if (!options.background && !options.catalogUpdate) {
        setHistorySessions([]);
        setHistorySelectedSessionId("");
        setHistoryPreview(emptyPreview());
      }
      ...
      if (options.refresh === true && historyManualRefreshRequestRef.current === request) {
        historyManualRefreshRequestRef.current = null;
      }
    }
  }
  ...
  async function handleLoadMoreHistorySessions(nextState: LocalState | null = state) {
    ...
    if (historyPrefetchedSessions.length > 0) {
      const combinedSessions = mergeSessionSummaries(historySessions, historyPrefetchedSessions);
      setHistorySessions(combinedSessions);
      setHistoryHasMore(historyPrefetchedHasMore);
      setHistoryNextCursor(historyPrefetchedNextCursor);
      setHistoryPrefetchedSessions([]);
      setHistoryPrefetchedHasMore(false);
      setHistoryPrefetchedNextCursor(undefined);
      historyPrefetchedCursorRef.current = null;
      latestHistoryPrefetchRequestRef.current = null;
      if (historyPrefetchedHasMore && historyPrefetchedNextCursor) {
        void prefetchHistorySessions(nextState, historyPrefetchedNextCursor, combinedSessions.length);
      }
      return;
    }
  ...
  useEffect(() => {
    ...
    if (sameSessionRequest(profilesSessionsHydratedRequestRef.current, currentProfilesSessionsRequest)) {
      return;
    }
    if (applyCachedProfilesSessions(currentProfilesSessionsRequest)) {
      return;
    }
    const reusedSessions = applyReusableProfilesSessions(currentProfilesSessionsRequest);
    if (!reusedSessions && (profilesSessions.length > 0 || profilesSelectedSessionId || !profilesSessionsLoading)) {
      clearProfilesSessionsState({ loading: true });
    }

    const run = () => void handleLoadProfilesSessions(state, currentProfilesSessionsRequest.cwd, { source: "auto" });
  ...
  useEffect(() => {
    if (!window.profileManager?.onSessionCatalogUpdated) {
      return;
    }
    return window.profileManager.onSessionCatalogUpdated((update) => {
      if (update.provider !== state?.selected_provider) {
        return;
      }
      setProfilesCatalogState(update.catalog_state);
      setHistoryCatalogState(update.catalog_state);
      const previousRevision = sessionCatalogRevisionRef.current[update.provider] ?? 0;
      if (update.revision <= previousRevision) {
        return;
      }
      sessionCatalogRevisionRef.current[update.provider] = update.revision;
      if (activeTab === "profiles" && currentProfilesSessionsRequest) {
        if (
          profilesManualRefreshRequestRef.current
          && sameSessionRequest(profilesManualRefreshRequestRef.current, currentProfilesSessionsRequest)
        ) {
          return;
        }
      ...
      } else if (activeTab === "sessions" && sessionsView !== "favorites") {
        const historyRequest = state ? buildHistorySessionsRequest(state) : null;
        if (
          historyRequest
          && historyManualRefreshRequestRef.current
          && sameSessionRequest(historyManualRefreshRequestRef.current, historyRequest)
        ) {
          return;
        }
        void handleLoadHistorySessions(state, { background: true, catalogUpdate: true });
      }
    });
  }, [
    activeTab,
  ...
  useEffect(() => {
    if (
      ...
      || sessionsView === "favorites"
      || !state?.selected_profile_key
      || (activeTab === "profiles" && (profilesSessionsLoading || profilesSessionsUninitialized))
    ) {
      return;
    }

    const request = buildHistorySessionsRequest(state);
    if (
      sameSessionRequest(historyHydratedRequestRef.current, request)
      || sameSessionRequest(historyLoadingRequestRef.current, request)
      || sameSessionRequest(historyWarmupRequestRef.current, request)
    ) {
      return;
    }

    historyWarmupRequestRef.current = request;
    const run = () => {
      if (!sameSessionRequest(historyWarmupRequestRef.current, request)) {
        return;
      }
      void handleLoadHistorySessions(state, { background: true }).finally(() => {
        if (sameSessionRequest(historyWarmupRequestRef.current, request)) {
          historyWarmupRequestRef.current = null;
        }
      });
  ...
  const globalPermissions = useMemo(
    () => {
      const permissions = normalizeGlobalPermissions(state?.global_settings.permissions);
      return selectedProvider === "codex" ? permissions.codex : permissions.claude;
    },
  ...
  const handleAdvancedModelMappingChange = useCallback((next: AdvancedModelMapping) => {
    setDraftAdvancedModelMapping(next);
    const snapshot = {
      ...currentDraftSnapshot(),
      advancedModelMapping: next,
    };
    void persistReasoningEffortOnlyChange(snapshot);
  }, [
    draftName,
    draftUrl,
    draftKey,
  ...
  const stableHandleRunCheckin = useEventCallback(handleRunCheckin);
  const stableHandleRunEnabledCheckins = useEventCallback(handleRunEnabledCheckins);
  const stableHandleToggleAutoCheckin = useEventCallback(handleToggleAutoCheckin);
  const stableHandleOpenManualCheckin = useEventCallback(handleOpenManualCheckin);
  const stableHandleProfilesLaunchMonitorModeChange = useEventCallback(handleProfilesLaunchMonitorModeChange);
  ...
  const stableHandleSelectProfilesSession = useEventCallback(handleSelectProfilesSession);
  const stableHandleSelectHistorySession = useEventCallback(handleSelectHistorySession);
  ...
  const stableRunCheckinAction = useCallback(() => void stableHandleRunCheckin(), [stableHandleRunCheckin]);
  const stableRunEnabledCheckinsAction = useCallback(
    () => void stableHandleRunEnabledCheckins(),
    [stableHandleRunEnabledCheckins],
  );
  const stableToggleAutoCheckinAction = useCallback(
    (enabled: boolean) => void stableHandleToggleAutoCheckin(enabled),
    [stableHandleToggleAutoCheckin],
  );
  const stableOpenManualCheckinAction = useCallback(
    () => void stableHandleOpenManualCheckin(),
    [stableHandleOpenManualCheckin],
  );
  ...
  return (
    <div className="app-shell-v2">
      ...
      {/* ===== PROFILES Tab ===== */}
      {activeTab === "profiles" && (
        <Suspense fallback={<PageLoading message="正在加载 Profiles 页面..." />}>
          <ProfilesPageComponent
            ...
            balanceTestProps={{
              ...
              checkinAvailable: Boolean(currentCheckinAccount),
              autoCheckinEnabled: currentCheckinAccount?.auto_checkin_enabled === true,
              enabledAccountCount: enabledCheckinAccountCount,
              actionBusy: checkinActionBusy || anyCheckinRunning,
              batchRunning: checkinBatchBusy,
              onCheckin: stableRunCheckinAction,
              onRunEnabledCheckins: stableRunEnabledCheckinsAction,
              onToggleAutoCheckin: stableToggleAutoCheckinAction,
              onOpenManualCheckin: stableOpenManualCheckinAction,
            }}
            profileEditProps={{
              draft: profileDraft,
              globalPermissions,
              ...
              onAdvancedModelMappingChange: handleAdvancedModelMappingChange,
            ...
            launchPanelProps={{
              ...
              onMonitorModeChange: stableHandleProfilesLaunchMonitorModeChange,
              disabled: isBusy || !state?.selected_profile_key,
              ...
              sessionsRefreshing: profilesSessionsRefreshing,
              sessionsUninitialized: profilesSessionsUninitialized,
              sessionsCatalogState: profilesCatalogState,
              selectedSessionId: profilesSelectedSessionId,
              favoriteSessionKeys: sessionFavoriteKeys,
              getSessionFavoriteKey,
              onSelectSession: stableHandleSelectProfilesSession,
      ...
      {/* ===== SKILLS Tab ===== */}
      {activeTab === "skills" && (
        <Suspense fallback={<PageLoading message="正在加载 Skills 页面..." />}>
      ...
      {/* ===== SESSIONS Tab ===== */}
      {activeTab === "sessions" && (
        <Suspense fallback={<PageLoading message="正在加载会话页面..." />}>
          <SessionsPageComponent
            ...
            sessionListProps={{
              ...
              onSelect: stableHandleSelectHistorySession,
              ...
              isRefreshing: sessionsView === "favorites" ? false : historyIsRefreshing,
              hasMoreSessions: sessionsView === "favorites" ? false : historyHasMore,
              catalogState: sessionsView === "favorites" ? "ready" : historyCatalogState,
      ...
      {/* ===== SETTINGS Tab ===== */}
      {activeTab === "settings" && (
        <Suspense fallback={<PageLoading message="正在加载设置页面..." />}>
