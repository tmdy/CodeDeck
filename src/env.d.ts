...
declare global {
  ...
  interface Window {
    ...
    profileManager?: {
      ...
      // Sessions
      listSessions: (request: ListSessionsRequest) => Promise<SessionPageResult | unknown[]>;
      getSessionDetail: (request: GetSessionDetailRequest) => Promise<unknown>;
      refreshSessions: (provider: string) => Promise<SessionRefreshResult | void>;
      onSessionCatalogUpdated?: (callback: (update: {
        provider: string;
        revision: number;
      ...
      // Check-in
      runCheckin?: (baseUrl: string, sessionId: string) => Promise<CheckinResult>;
      runEnabledCheckins?: () => Promise<CheckinBatchSummary>;
      openCheckinVerification?: (
        baseUrl: string,
        sessionId: string,
      ) => Promise<CheckinVerificationResult>;
      getCheckinSnapshot?: () => Promise<CheckinSnapshot>;
      ...
      onCheckinProgress?: (
        callback: (accountKey: string, state: CheckinAccountState) => void,
      ) => () => void;
      onCheckinScheduleChanged?: (callback: (nextScheduledAt: string) => void) => () => void;
