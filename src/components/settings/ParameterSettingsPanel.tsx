...
export function ParameterSettingsPanel({
  ...
  return (
    <div className="parameter-settings">
      ...
      <GlassCard title="终端与监控设置">
        <label className="checkbox-label">
          <input
            type="checkbox"
            checked={settings.terminal?.auto_continue_on_failure ?? true}
            onChange={(e) =>
              onChange({
                terminal: {
                  ...settings.terminal,
        ...
        <label>
          ...
          <input
            type="number"
            min={-1}
            value={settings.terminal?.auto_continue_limit ?? 1}
            onChange={(e) =>
              onChange({
                terminal: {
                  ...settings.terminal,
        ...
        <label>
          ...
          <input
            ...
            min={0}
            step={0.1}
            value={(settings.terminal?.auto_continue_interval_ms ?? 0) / 1000}
            onChange={(e) =>
              onChange({
                terminal: {
                  ...settings.terminal,
                  auto_continue_interval_ms: Math.max(0, Math.round((parseFloat(e.target.value) || 0) * 1000)),
                },
              })
            }
            disabled={disabled}
        ...
        <label>
          自动继续关键词
          <textarea
            value={(settings.terminal?.auto_continue_keywords ?? []).join("\n")}
            onChange={(e) =>
              onChange({
                terminal: {
                  ...settings.terminal,
        ...
        <label>
          自动继续输入文本
          <input
            value={settings.terminal?.auto_continue_prompt ?? "继续"}
            onChange={(e) =>
              onChange({
                terminal: {
                  ...settings.terminal,
