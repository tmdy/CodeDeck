...
describe("App page containers", () => {
  it("renders the Profiles page layout with editor and launch panels", () => {
      ...
      <ProfilesPage
        ...
        balanceTestProps={{
          state: {
            ...
            endpoint: "https://relay.example.com/user/balance",
            finished_at_display: "2026/07/01 11:00:00",
          },
          sessionHint: "API Key 自动",
          onTest: vi.fn(),
        }}
        profileEditProps={{
          draft: {
    ...
    expect(html.indexOf("站点后台会话")).toBeLessThan(html.indexOf("<label>后台会话"));
    expect(html.indexOf("<label>后台会话")).toBeLessThan(html.indexOf("来源: https://relay.example.com/user/balance"));
    expect(html).toContain("最近检测: 2026/07/01 11:00:00");
  });
