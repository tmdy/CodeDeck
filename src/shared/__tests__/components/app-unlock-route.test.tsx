...
describe("App startup route", () => {
  ...
  it("should render lazy page loading inside the main content area", async () => {
    const appSource = normalizeNewlines(await readFile(path.join(process.cwd(), "src", "App.tsx"), "utf8"));
    const css = normalizeNewlines(await readFile(path.join(process.cwd(), "src", "styles.css"), "utf8"));

    expect(appSource).toContain("function PageLoading");
    expect(appSource).toContain('className="page-loading-screen"');
    expect(appSource).toContain('<Suspense fallback={<PageLoading message="正在加载 Profiles 页面..." />}>');
    expect(appSource).not.toContain('<Suspense fallback={<StartupLoading message="正在加载 Profiles 页面..." />}>');
    expect(css).toContain(".page-loading-screen {\n  display: flex;");
    expect(css).toContain("  width: 100%;");
    expect(css).toContain("  min-height: calc(100vh - 92px);");
    expect(css).not.toContain(".page-loading-screen {\n  display: flex;\n  justify-content: center;\n  align-items: center;\n  min-height: 100vh;\n  width: 100vw;");
  });

  it("should schedule Profiles session restore loading on the next animation frame", async () => {
    const source = normalizeNewlines(await readFile(path.join(process.cwd(), "src", "App.tsx"), "utf8"));
    const effectStart = source.indexOf('if (activeTab !== "profiles")');
    const effectEnd = source.indexOf("currentProfilesSessionsRequest?.provider,", effectStart);
    const effectBlock = source.slice(effectStart, effectEnd);

    expect(effectBlock).toContain("window.requestAnimationFrame(run)");
    expect(effectBlock).toContain("window.cancelAnimationFrame(id)");
    expect(effectBlock).not.toContain("requestIdleCallback");
    expect(effectBlock).not.toContain("setTimeout(run, 800)");
  });

  it("should not warm global history while Profiles restore sessions are still loading", async () => {
    const source = normalizeNewlines(await readFile(path.join(process.cwd(), "src", "App.tsx"), "utf8"));
    const warmupStart = source.indexOf('startupPhase !== "ready"');
    const warmupEnd = source.indexOf("historyWarmupRequestRef.current = request;", warmupStart);
    const warmupGuardBlock = source.slice(warmupStart, warmupEnd);

    expect(warmupGuardBlock).toContain('activeTab === "profiles"');
    expect(warmupGuardBlock).toContain("profilesSessionsLoading || profilesSessionsUninitialized");
  });
