...
describe("SessionPicker", () => {
  ...
  it("renders a favorite control beside the selected session heading", async () => {
    const container = document.createElement("div");
    document.body.appendChild(container);
    const root = createRoot(container);
    ...
    await act(async () => {
      root.render(
        <SessionPicker
          ...
          favoriteSessionKeys={new Set(["favorite-session-long"])}
          getFavoriteKey={(session) => `favorite-${session.session_id}`}
    ...
    const summary = container.querySelector(".session-picker-selected-summary");
    const favoriteButton = summary?.querySelector<HTMLButtonElement>(
      'button[aria-label="取消收藏会话：可以再加一句更适合 Claude Code 的执行指令，并且这是一段非常长的会话预览文本"]',
    );
    ...
    expect(onToggleFavorite).toHaveBeenCalledWith(expect.objectContaining({ session_id: "session-long" }));
