...
describe("LaunchService", () => {
  it("should launch Claude with the raw selectedModelId and custom model env vars by default", () => {
    ...
    expect(preview.command).toContain("--model deepseek-v4-pro --permission-mode manual");
  ...
  it("should inject global proxy while ignoring removed legacy traffic controls", () => {
    const profile: Profile = {
      provider: "claude",
      name: "Official",
      url: "https://api.anthropic.com",
    ...
    const service = new ProfileService([profile], new MemoryStateAccessor({
      ...
      } as unknown as LocalState["global_settings"],
    }));
    const launchService = new LaunchService(service, {
      getModelMappingsState: () => makeMappings(),
      codexProfilesRoot: "C:/tmp/codex-profiles",
    ...
    expect(plan.env.DISABLE_ERROR_REPORTING).toBeUndefined();
    expect(plan.env.CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC).toBeUndefined();
    expect(plan.env.DISABLE_NON_ESSENTIAL_MODEL_CALLS).toBeUndefined();
  ...
  it("should ignore removed legacy co-authored-by settings in managed settings", () => {
    const profile: Profile = {
      provider: "claude",
      name: "Official",
      url: "https://api.anthropic.com",
    ...
    expect(plan.valid).toBe(true);
    expect(plan.claudeSettings?.content).not.toContain("includeCoAuthoredBy");
  });
  ...
  it("should pass structured Claude effort as a session CLI argument", () => {
    const profile: Profile = {
      provider: "claude",
      name: "DeepSeek Pro",
      url: "https://api.deepseek.com/anthropic",
    ...
    expect(plan.valid).toBe(true);
    expect(plan.command).toContain("--effort max");
    expect(plan.commandArgs).toEqual(expect.arrayContaining(["--effort", "max"]));
    expect(plan.env.CLAUDE_CODE_EFFORT_LEVEL).toBeUndefined();
  });
  ...
  it("should only inject Claude family alias mapping when the profile explicitly enables it", () => {
    ...
    expect(preview.command).toContain('--resume "session-123" --model kimi-k2 --permission-mode manual');
  ...
  it("should inject all Claude aliases when single-model compatibility mode is enabled", () => {
    ...
    expect(plan.command).toContain("--model glm-5.1 --permission-mode manual");
  ...
  it("should pass structured Codex reasoning effort as a config override", () => {
    const profile: Profile = {
      provider: "codex",
      name: "OpenAI",
      url: "https://api.openai.com/v1",
    ...
    expect(plan.valid).toBe(true);
    expect(plan.command).toBe("codex --profile site-a5f93b45c3f2dd18 -c model_reasoning_effort=ultra");
    expect(plan.commandArgs).toEqual([
      "--profile",
      "site-a5f93b45c3f2dd18",
      "-c",
  ...
  it("should pass custom Codex auto-continue keywords and unlimited limit from global settings", () => {
    ...
    const service = new ProfileService([profile], new MemoryStateAccessor({
      parameter_settings: {
        ...defaultLocalState().parameter_settings,
        terminal: {
          ...defaultLocalState().parameter_settings.terminal,
          auto_continue_limit: -1,
          auto_continue_keywords: ["high demand", "排队中", "服务繁忙"],
          auto_continue_interval_ms: 2_500,
        },
      },
    }));
    const launchService = new LaunchService(service, {
  ...
  it("should honor monitored terminal override for Claude plans", () => {
    const profile: Profile = {
      provider: "claude",
      name: "Official",
      url: "https://api.anthropic.com",
    ...
    const preview = launchService.previewForRequest({
      profile_key: itemKey(profile),
      provider: "claude",
      runtime_settings: makeRuntime({ command_base: "claude" }),
      terminal_mode: "monitored",
    ...
    expect(preview.terminalSummary).toBe("Claude 终端：受监控独立窗口");
  });

  it("should use the shared monitored terminal default for Claude when no override is provided", () => {
    const profile: Profile = {
      provider: "claude",
      name: "Official",
      url: "https://api.anthropic.com",
  ...
  it("should launch Claude without model arguments when selected model id is empty", () => {
    ...
    expect(plan.valid).toBe(true);
    expect(plan.command).toContain('claude --setting-sources "project,local" --settings "C:/tmp/claude-runtime/permissions/claude-permissions-');
    expect(plan.command).toContain("--permission-mode manual");
