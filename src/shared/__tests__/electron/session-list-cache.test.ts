...
describe("session-list-cache", () => {
  ...
  it("keeps cursor pages in separate cache entries", () => {
    const homes: CodexSessionHome[] = [{ kind: "global_codex", home: "C:/Users/example/.codex" }];
    const firstPage = buildSessionListCacheKey(createRequest({ limit: 20, offset: 0 }), homes);
    const nextPage = buildSessionListCacheKey(createRequest({
      limit: 20,
      offset: 0,
      cursor: JSON.stringify([1_700_000_000_000, "session-20", "global_codex"]),
    }), homes);
