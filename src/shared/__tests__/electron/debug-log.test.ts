...
describe("createAppLogger", () => {
  ...
  it("keeps a short diagnostic run id searchable", () => {
    expect(sanitizeLogContext({ run_id: "abcdef123456" })).toEqual({
      run_id: "abcdef123456",
    });
  });
...
describe("createIpcHandlerLogger", () => {
  ...
  it("can suppress routine activity while preserving error logs", async () => {
    const logger = {
      debug: vi.fn(),
      info: vi.fn(),
      warn: vi.fn(),
    ...
    handleIpc("terminal:resize", () => undefined, { logActivity: false });
    const wrapped = register.mock.calls[0][1] as (event: unknown, ...args: unknown[]) => Promise<unknown>;
    await wrapped({}, "session-id", 120, 40);
