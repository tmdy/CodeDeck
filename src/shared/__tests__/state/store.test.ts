...
describe("LocalStateStore", () => {
  ...
  it("should persist non-sensitive automatic check-in state", async () => {
    const store = await makeStore();
    const state = defaultLocalState();
    state.checkin_next_scheduled_at = "2026-07-14T09:15:00.000Z";
    state.checkin_states_by_account["https://relay.example.com::account-a"] = {
      status: "failed",
      trigger: "automatic",
      message: "签到失败，请稍后重试",
      error_code: "business_rejected",
      diagnostic_run_id: "abcdef123456",
      last_attempt_at: "2026-07-14T09:15:00.000Z",
      last_auto_attempt_local_date: "2026-07-14",
    };

    await store.save(state);
    const loaded = await store.load();

    expect(loaded.checkin_next_scheduled_at).toBe("2026-07-14T09:15:00.000Z");
    expect(loaded.checkin_states_by_account["https://relay.example.com::account-a"])
      .toMatchObject({
        status: "failed",
        error_code: "business_rejected",
        diagnostic_run_id: "abcdef123456",
