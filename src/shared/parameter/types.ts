...
export interface TerminalSettings {
  ...
  auto_continue_interval_ms: number;
}
...
export function normalizeParameterSettings(settings?: Partial<ParameterSettings> | null): ParameterSettings {
  ...
  const rawCodex = settings?.cli_settings?.codex as (Partial<CodexCLISettings> & Partial<TerminalSettings>) | undefined;
  const rawTerminal = settings?.terminal;
  ...
  normalized.terminal.terminal_mode = normalizeCodexTerminalMode(
    normalized.terminal.terminal_mode,
  );
  const rawAutoContinueLimit = Math.floor(Number(normalized.terminal.auto_continue_limit) || 1);
  normalized.terminal.auto_continue_limit = rawAutoContinueLimit === -1
    ? -1
    : Math.max(1, rawAutoContinueLimit);
  normalized.terminal.auto_continue_interval_ms = Math.max(
    0,
    Math.floor(Number(normalized.terminal.auto_continue_interval_ms) || 0),
  );
  normalized.terminal.auto_continue_prompt =
    normalized.terminal.auto_continue_prompt?.trim() || "继续";
  normalized.terminal.auto_continue_keywords = normalizeAutoContinueKeywords(
    normalized.terminal.auto_continue_keywords,
  );
