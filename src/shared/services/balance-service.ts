...
import { buildManagementCookieCandidates } from "./management-auth.js";
...
const AIHUBMIX_HOSTS = new Set([
  "aihubmix.com",
  "www.aihubmix.com",
  "console.aihubmix.com",
]);
const SHAREDCHAT_HOSTS = new Set(["new.sharedchat.cc"]);
...
function extractLikelyUserIds(token: string): number[] {
  ...
  for (const candidate of buildManagementCookieCandidates(raw)) {
    const match = candidate.headerValue.match(/(?:^|;\s*)session=([^;]+)/i);
    if (match?.[1]) {
...
function isAIHubMixBaseUrl(baseUrl: string): boolean {
  return AIHUBMIX_HOSTS.has(normalizeHost(baseUrl));
}

function isSharedChatBaseUrl(baseUrl: string): boolean {
  return SHAREDCHAT_HOSTS.has(normalizeHost(baseUrl));
}

function isVoApiV2BaseUrl(baseUrl: string): boolean {
  return /vo[-_\s]?api[-_\s]?v?2|voapi[-_\s]?2/i.test(normalizeUrlForMatching(baseUrl));
}
...
function detectManagementFamilyByUrl(baseUrl: string): ManagementFamily | null {
  ...
  if (
    /new[-\s]?api|anyrouter|voapi|neo[-\s]?api|rix[-\s]?api|super[-\s]?api|v[-\s]?api|wong/i
      .test(haystack)
  ) {
...
function detectManagementFamilyByText(bodyText: string): ManagementFamily | null {
  ...
  if (
    /new[-\s]?api|anyrouter|voapi|neo[-\s]?api|rix[-\s]?api|super[-\s]?api|v[-\s]?api|wong|new-api-user|veloera-user|user-id|x-api-user|x-user-id/i
      .test(haystack)
  ) {
    return "new_api_family";
  }
...
function detectPlatformFamily(baseUrl: string): PlatformFamily {
  ...
  if (isAIHubMixBaseUrl(baseUrl)) {
    return "aihubmix";
  }
  if (isSharedChatBaseUrl(baseUrl)) {
    return "sharedchat";
  }
...
function voApiV2PayloadLooksSupported(payload: unknown): boolean {
  if (!isRecord(payload) || typeof payload.code !== "number") {
    return false;
  }
  if (isRecord(payload.data)) {
    return recordHasAnyKey(payload.data, [
      "basicBalance",
      "bindBalance",
      "usedBasicBalance",
      "usedBindBalance",
  ...
  return recordHasAnyKey(payload, ["msg", "message", "token", "rid"]);
}
...
function parseVoApiV2BalanceState(
  context: BalanceContext,
  endpoint: string,
  payload: unknown,
): BalanceCheckState {
  ...
  const code = coerceNumber(payload.code);
  if (code !== null && code !== 0) {
    return supportedFailureState(
      context,
      endpoint,
      message || (code === 2 ? SESSION_EXPIRED_MESSAGE : "余额接口返回失败状态"),
    );
  }

  const data = isRecord(payload.data) ? payload.data : payload;
  const basic = coerceNumber(data.basicBalance) ?? 0;
  const bound = coerceNumber(data.bindBalance) ?? 0;
  const usedBasic = coerceNumber(data.usedBasicBalance) ?? 0;
  const usedBound = coerceNumber(data.usedBindBalance) ?? 0;
  const hasBalance = recordHasAnyKey(data, [
    "basicBalance",
    "bindBalance",
    "usedBasicBalance",
    "usedBindBalance",
  ...
  const remaining = basic + bound;
  const used = usedBasic + usedBound;
  return successState(
    context,
    endpoint,
    [buildUsdItem(remaining, remaining + used, used)],
  );
}

function parseSharedChatBalanceState(
  context: BalanceContext,
  endpoint: string,
  payload: unknown,
): BalanceCheckState {
  ...
  if (coerceNumber(payload.code) !== 1) {
    return supportedFailureState(
      context,
      endpoint,
      extractResponseMessage(payload) || SESSION_EXPIRED_MESSAGE,
    );
  }

  const root = isRecord(payload.data) ? payload.data : null;
  const codex = root && isRecord(root.codex) ? root.codex : null;
  const subscription = codex && isRecord(codex.subscriptions) ? codex.subscriptions : null;
  if (!subscription) {
    return supportedFailureState(context, endpoint, "站点返回了非预期格式，暂未识别 Codex 订阅余额");
  }

  const remaining = coerceNumber(subscription.remainingAmount);
  const used = coerceNumber(subscription.usedAmount);
  const total = coerceNumber(subscription.amountLimit)
    ?? (remaining !== null && used !== null ? remaining + used : null);
  if (remaining === null && total === null) {
    return supportedFailureState(context, endpoint, "站点返回了非预期格式，暂未识别 Codex 订阅余额");
  }

  const label = typeof subscription.subTypeName === "string" && subscription.subTypeName.trim()
    ? subscription.subTypeName.trim()
    : "Codex";
  return successState(
    context,
    endpoint,
    [buildUsdItem(remaining ?? total, total, used, label)],
  );
}
...
function describeNetworkError(error: unknown): string {
  ...
  if (/SSL|TLS|CERT/i.test(`${code} ${causeMessage} ${message}`)) {
    return "TLS 连接失败，请检查系统代理、证书或站点防护";
  }
  if (/fetch failed/i.test(message) || /socket|connect/i.test(`${code} ${causeMessage}`)) {
    return "网络连接失败，请检查系统代理或站点是否可访问";
  }
  return message || "网络错误 / 超时";
}
...
export class BalanceService {
  ...
  async query(
    ...
    if (familyHint === "aihubmix") {
      return this.queryAIHubMix(context, apiKey, timeoutMs, resolvedSession);
    }
    if (familyHint === "sharedchat") {
      return this.querySharedChat(context, apiKey, timeoutMs, resolvedSession);
    }
    if (familyHint === "voapi_v2") {
      return (await this.queryVoApiV2(context, apiKey, timeoutMs, resolvedSession, true))
  ...
  private async queryAIHubMix(
    context: BalanceContext,
    apiKey: string,
    timeoutMs: number,
    resolvedSession: SiteBalanceSession | null,
  ): Promise<BalanceCheckState> {
    const endpoint = `${AIHUBMIX_API_ORIGIN}/api/user/self`;
    const authToken = (resolvedSession?.access_token ?? apiKey).trim();
    if (!authToken) {
      return supportedFailureState(context, endpoint, ACCESS_TOKEN_REQUIRED_MESSAGE);
    }
    ...
    if (!response.ok || payloadSignalsFailure(response.payload)) {
      const message = extractResponseMessage(response.payload) || response.bodyText;
      return supportedFailureState(
        context,
        endpoint,
        formatManagementFailureMessage(message, authToken, "bearer", true)
          || `余额接口请求失败（HTTP ${response.status}）`,
      );
    }

    return parseManagementBalanceState(
      context,
      endpoint,
      response.payload,
      "new_api_family",
  ...
  private async querySharedChat(
    context: BalanceContext,
    apiKey: string,
    timeoutMs: number,
    resolvedSession: SiteBalanceSession | null,
  ): Promise<BalanceCheckState> {
    const sharedChatOrigin = new URL(context.base_url).origin;
    const endpoint = `${sharedChatOrigin}/frontend-api/vibe-code/quota`;
    const authToken = (resolvedSession?.access_token ?? apiKey).trim();
    if (!authToken) {
      return supportedFailureState(context, endpoint, "该站点余额接口需要已登录 Cookie");
    }

    let lastResponse: RequestResult | null = null;
    for (const candidate of buildManagementCookieCandidates(authToken)) {
      const response = await this.requestJson(
        endpoint,
        authToken,
        timeoutMs,
      ...
      if (response.ok && isRecord(response.payload) && coerceNumber(response.payload.code) === 1) {
        return parseSharedChatBalanceState(context, endpoint, response.payload);
      }
    }

    if (lastResponse?.error) {
      return supportedFailureState(context, endpoint, lastResponse.error);
    }
    return supportedFailureState(
      context,
      endpoint,
      extractResponseMessage(lastResponse?.payload) || SESSION_EXPIRED_MESSAGE,
    );
  }

  private async queryVoApiV2(
    context: BalanceContext,
    apiKey: string,
    timeoutMs: number,
    resolvedSession: SiteBalanceSession | null,
    ...
    const endpoint = `${context.base_url}/api/user/info`;
    const authToken = (resolvedSession?.access_token ?? apiKey).trim();
    if (!authToken) {
    ...
    const matched = assumeFamily || voApiV2PayloadLooksSupported(response.payload);
    if (!matched) {
      return null;
    }
    if (response.error) {
    ...
    return parseVoApiV2BalanceState(context, endpoint, response.payload);
  }
  ...
  private async queryManagementFamily(
    ...
    if (shouldRetryWithManagementSessionCookie(bearerResponse, authToken, resolvedSession)) {
      for (const cookieCandidate of buildManagementCookieCandidates(authToken)) {
        const cookieHeader = cookieCandidate.headerValue;
        const triedUserIds = new Set<string>();
